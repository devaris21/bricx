                                            ...
                                            <?php foreach ($pro->fourni("produit", ["isActive ="=>Home\TABLE::OUI]) as $key => $produit) {
                                                ...
                                                <li class="list-group-item">
                                                    ...
                                                    <?php foreach (Home\EMBALLAGE::getAll() as $key => $emballage) {
                                                        $a = $produit->enEntrepot(Home\PARAMS::DATE_DEFAULT, dateAjoute(1), $emballage->id, $agence->id);
                                                        if ($a > 0) { ?>
                                                            <span class="float-right">
                                                                <small title="en agence" class="gras <?= ($a * $emballage->nombre() > $params->ruptureStock)?"":"text-red clignote" ?>"><?= start0($a) ?></small> <img style="height: 15px" src="<?= $this->stockage("images", "emballages", $emballage->image)  ?>"> |
                                                            </span>
                                                        <?php }
                                                    } ?>                                                               
                                                </li>
